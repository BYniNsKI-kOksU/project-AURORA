...
def prompt_sky_map_background(*, default: str = "plain") -> str:
    ...
    if not is_main_process:
        return get_sky_map_background(env_value, default=default_background)
    if env_value:
        background = get_sky_map_background(
            env_value,
    ...
    while True:
        ...
        try:
            response = console.prompt(
                f"Background map [1-8] (Enter = {default_number})"
            ).strip()
        except EOFError:
            response = ""
        try:
            ...
            console.warning("Enter a background-map number from 1 to 8")
            continue
        os.environ["AURORA_SKY_MAP_BACKGROUND"] = background
        return background
