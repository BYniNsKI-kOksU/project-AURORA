...
o wariant mapy. Menu pokazuje wyłącznie warianty 16K, których pliki istnieją
w `maps/`, i zachowuje ich stałe numery 1–8. Wybrany plik PNG jest bezpośrednio
używany jako tło każdej klatki animacji.
...
in all-sky mode it offers only the pre-rendered 16K combinations that actually
exist in `maps/`, while preserving their stable option numbers from 1 to 8.
The selected PNG is used directly as the background of every animation frame;
these lines are never redrawn for individual frames.
